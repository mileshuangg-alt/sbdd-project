...
def parse_vinardo_score(block):
    for line in block.splitlines():
        if line.startswith("REMARK minimizedAffinity"):
            return float(line.split()[2])

    raise AssertionError("Missing REMARK minimizedAffinity record.")
...
def reconstruct_poses():
    ...
    if pose_count != EXPECTED_POSES:
        raise AssertionError(
            f"Expected {EXPECTED_POSES} parsed poses, found {pose_count}."
        )
...
def main():
    ...
    scores = [parse_vinardo_score(block) for block in blocks]
    index_map = assert_index_maps_are_stable(blocks)

    native = load_native_ligand()
    pose_mol = reconstruct_poses()
    ...
    rmsds = calculate_rmsd_by_pose(pose_mol, native)
    ...
    for conformer_index in range(EXPECTED_POSES):
        ...
        result = {
            "rank": rank,
            "vinardo_score": scores[conformer_index],
    ...
    print("Candidate-3 3REY / XAC self-redocking validation")
    print("=" * 72)
    ...
    print()
    print(
        "Rank,VinardoScore,RMSD,Phe168Recovered,"
